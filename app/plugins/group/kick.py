...
__module__ = {
    ...
    "name": "kick",
    "commands": ["kick", "skick", "dkick"], # list of commands including aliases

    "description": "Kick chat member (they can rejoin)! E.g. `/kick @username reason` or reply a member with `/kick reason`!",
    "category": "group", # check app/__init__.py for HELP_MENU_CATEGORIES
    "button_name": "Kick", # Help menu button name (Note: Leaving blank or None will result in no button on help menu)
...
async def func_(_, message: Message):
    ...
    if victim.id == bot.me.id:
        return await message.reply_text(
            "Huh, I'm not going to kick myself!"
        )
    ...
    try:
        ...
        if not is_silent:
            await message.reply_text(
                f"Kicked out {victim.mention} from this chat." + (f"\nReason: {reason}" if reason else "")
            )
